...
const TextWriter = (input) => {
    ...
    let id = `id${count}`
    console.log(count)
    if(document.getElementById(`id${count - 1}`)){document.getElementById(`id${count - 1}`).remove()
    var textcontainer = document.createElement("span")
    textcontainer.id = id
    document.getElementById("animtextcontainer").appendChild(textcontainer)}
    ...
    // chekc if text isn't finished yet
        if (i < (text.length)) {
            ...
            if(document.getElementById(id)){document.getElementById(id).innerHTML = text.substring(0, i+1)};
